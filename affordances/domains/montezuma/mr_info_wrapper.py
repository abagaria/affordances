...
class MontezumaInfoWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):  # TODO: load the frames from disk
        if 'ram' in kwargs:
            ram = kwargs['ram']
            s0, info0 = self.set_player_ram(ram)
            s0 = self.tensor2lz(kwargs['state'])
        else:
            s0 = self.env.reset(**kwargs)
            info0 = {}
        self.num_lives = self.get_num_lives(self.get_current_ram())
        info = self.get_current_info(info=info0)
        info["needs_reset"] = False
        return s0, info
    
    @staticmethod
    def tensor2lz(tensor_obs):
        frames = []
        for i in range(tensor_obs.shape[0]):
            frame = tensor_obs[i]
    ...
    def set_player_ram(self, ram_state):
        state_ref = self.get_current_ale().cloneState()
        self.get_current_ale().deleteState(state_ref)
        
        new_state_ref = self.get_current_ale().decodeState(ram_state)
        self.get_current_ale().restoreState(new_state_ref)
        self.get_current_ale().deleteState(new_state_ref)
        for _ in range(2):
            obs, _, _, info = self.env.step(0)  # NO-OP action to update the RAM state
            obs, _, _, info = self.env.step(0)  # 2nd no-op for correct frame stack
        return obs, info
    ...
    def get_current_info(self, info, update_lives=False):
        ...
        info["player_pos"] = info["player_x"], info["player_y"]
    ...
    def get_current_ale(self):
        ...
        return self.environment.ale  # when the inner env is a dopamine env
